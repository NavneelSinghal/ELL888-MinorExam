...
# modification to the original solution where we can force distance between elements of separate classes to 0
def l2_degree_reg_known_classes(X, classes, forced_distance=1e9, dist_type='sqeuclidean', alpha=1, s=None,
                                step=0.5, w0=None, maxit=1000, rtol=1e-5, retall=False,
                                verbosity='NONE'):
    r"""
    Learn graph by regularizing the l2-norm of the degrees.
    We take into account the information about classes, i.e.,
    set distance between nodes in different classes to infinity.
    ...
    classes : list of lists
        classes[i] = list of nodes in class number i
    forced_distance : float
        Distance to be used for elements in different classes
    dist_type : string
        Type of pairwise distance between variables. See
        :func:`spatial.distance.pdist` for the possible options.
    alpha : float, optional
    ...
    Notes
    -----
    This is the problem proposed in [Dong et al., 2015], with the
    modifications as specified in the solution to the
    semi-supervised learning problem in the exam.
    ...
    k = len(classes)
    for i in range(k):
        for node_1 in classes[i]:
            for j in range(i + 1, k):
                for node_2 in classes[j]:
                    ni, nj = min(node_1, node_2), max(node_1, node_2)
                    z[N * ni + nj - ((ni + 2) * (ni + 1)) // 2] = forced_distance
